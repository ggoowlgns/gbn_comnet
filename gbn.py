...
class GBNsend(threading.Thread):
    ...
    def send_packet(self, pkt_type, data=b''):
        ...
        if self.nextseqnum < self.base+N:
            pkt = make_pkt(self.nextseqnum ,pkt_type ,data) #packet만들기
            self.sndpkt.append(pkt) #buffer에 넣기

            udt_send(self,pkt) #보내기
    ...
    def handle_ACK(self, packet):
        ...
        ack_num = get_seqnum(packet)

        for i in srange(self.base, ack_num): #이전까지 buffer에 있는거 제거
            del self.sndpkt[0]
    ...
    def run(self):
        ...
        while self.state != 'Closed':
            ...
            if self.state == 'Closing':     # FIN sent, then waiting for FINACK
                # your codes here
                if event == TIMEOUT:
                    self.retransmit()#FIN 다시 보냄
                elif event == Recv_ACK:
                    self.handle_ACK(rcvpkt)

                elif event == Recv_FINACK:
                    self.handle_ACK(rcvpkt)
                    if self.base == self.nextseqnum:
                        self.stop_timer()
                    self.state = 'Closed'
...
class GBNrecv(threading.Thread):
    ...
    def send_packet(self, pkt_type):
        ...
        self.sndpkt = make_pkt(self.expected_seqnum, pkt_type)
        udt_send(self, self.sndpkt)
    ...
    def run(self):
        ...
        while self.state != 'Closed':
            ...
            if self.state == 'Wait' :
                # your codes here
                if ((event == Recv_DATA) or (event == Recv_FIN)):
                    seq = get_seqnum(packet)
                    if ((seq < self.base+N) and (seq >= self.base)):

                        self.rcvpkt[get_seqnum(packet)] = packet
                    else:
                        self.retransmit()
                        continue #해당 packet 버림
                    self.expected_seqnum = self.rcvpkt.find_hole()
                    for seq in srange(self.base , self.expected_seqnum):
                        pkt = self.rcvpkt.popfront()
                        if get_type(pkt) == FIN:
                            self.start_timer(TIMEOUT_INTV * 2)
                            self.send_packet(FINACK)
                            self.state = 'Closing'
                        data = extract(pkt)
                        self.deliver(data)
                    self.base = self.rcvpkt.find_hole()
                    self.send_packet(ACK)

                    # if get_seqnum(packet) == self.expected_seqnum:
                    #     data = extract(packet)
                    #     self.deliver(data)
                    #     self.send_packet(ACK)
                ...
                #     self.send_packet(ACK)
                    # seq = get_seqnum(packet)
                    # if ((seq < self.base+N) and (seq >= self.base)):
                    #     self.rcvpkt[seq] = packet
                    #     self.expected_seqnum = self.rcvpkt.find_hole()
                    #     if self.base < self.expected_seqnum:
                    #         hole = self.rcvpkt.find_hole()
                    #         for seq in srange(self.base , hole):
                    #             pkt = self.rcvpkt.popfront()
                    #             data = extract(pkt)
                    #             self.deliver(data)
                    #         self.base = hole
                    #         self.send_packet(ACK)
                    #
                    #     elif (seq+1) == self.expected_seqnum:
                    #         self.deliver(b'')
                    #         self.send_packet(ACK)
                    #         self.start_timer(TIMEOUT_INTV * 2)
                    #         self.state = 'Closing'
